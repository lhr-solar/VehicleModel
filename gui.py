...
class SimulationGUI:
    ...
    def _create_ui(self):
        ...
        # Title
        title_label = ttk.Label(control_frame, text="Simulation Control", font=('Arial', 14, 'bold'))
        ...
        ttk.Label(control_frame, text="Simulation Parameters", font=('Arial', 11, 'bold')).grid(row=row, column=0, columnspan=2, pady=5)
        row += 1
        ...
        ttk.Label(control_frame, text="Output Parameters", font=('Arial', 11, 'bold')).grid(row=row, column=0, columnspan=2, pady=5)
        row += 1
        
        ttk.Label(control_frame, text="Select parameters to log & graph:", font=('Arial', 9)).grid(row=row, column=0, columnspan=2, sticky=tk.W, pady=(5, 2))
        row += 1
        ...
        ttk.Label(control_frame, text="Custom parameters (comma-separated):", font=('Arial', 9)).grid(row=row, column=0, columnspan=2, sticky=tk.W, pady=(10, 2))
        row += 1
        self.custom_params_entry = ttk.Entry(control_frame, width=30)
        self.custom_params_entry.grid(row=row, column=0, columnspan=2, sticky=tk.EW, pady=5)
        row += 1
        
        # Info label
        info_label = ttk.Label(control_frame, text="Note: GUI does not modify params.yaml", 
                               font=('Arial', 8, 'italic'), foreground='blue')
        info_label.grid(row=row, column=0, columnspan=2, pady=(10, 5))
        row += 1
        
        # Run button
        self.run_button = ttk.Button(control_frame, text="Run Simulation", command=self._run_simulation)
        self.run_button.grid(row=row, column=0, columnspan=2, pady=20)
        row += 1
        
        # Progress indicator
        self.progress_label = ttk.Label(control_frame, text="Status: Ready", foreground="green")
        self.progress_label.grid(row=row, column=0, columnspan=2, pady=5)
        row += 1
        
        # Console output
        ttk.Label(control_frame, text="Console Output:").grid(row=row, column=0, sticky=tk.W, pady=5)
        row += 1
        self.console = scrolledtext.ScrolledText(control_frame, width=40, height=10, wrap=tk.WORD)
    ...
    def _simulation_worker(self):
        try:
            ...
            log_params = [param for param, var in self.log_param_vars.items() if var.get()]
            ...
            if invalid_params:
                self._log(f"WARNING: {len(invalid_params)} invalid parameter(s) ignored: {', '.join(invalid_params)}")
            
            # Create graphs only for valid parameters
            if valid_params:
                self._create_graphs(valid_params)
                self.progress_label.config(text="Status: Complete (with warnings)" if invalid_params else "Status: Complete", 
                                          foreground="orange" if invalid_params else "green")
                self._log(f"Graphs generated successfully for {len(valid_params)} parameter(s)!")
            else:
                self._log("ERROR: No valid parameters to graph!")
                self.progress_label.config(text="Status: No valid data", foreground="red")
