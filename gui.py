...
class SimulationGUI:
    ...
    def _gs_display_config(self, index):
        ...
        if current_tab:
            for tab_id in self.gs_notebook.tabs():
                if self.gs_notebook.tab(tab_id, "text") == current_tab:
                    self.gs_notebook.select(tab_id)
                    break
    ...
    def _create_wp_tab(self, parent):
        ...
        row = 0
        ttk.Label(cf, text="Simulation", font=("Arial", 14, "bold")).grid(
